...
describe("commonSchemas", () => {
    it("validates correct EAN13 barcode", () => {
        expect(CommonSchemas.barcode.safeParse("4006381333931").success).toBe(
            true,
        );
    });
    it("rejects incorrect EAN13 barcode", () => {
        ...
        expect(CommonSchemas.barcode.safeParse("123456789012").success).toBe(
            false,
        );
    });
    ...
    it("rejects disposable email", () => {
        expect(
            CommonSchemas.email.safeParse("foo@mailinator.com").success,
        ).toBe(false);
    });
    it("rejects unsafe string (XSS)", () => {
        expect(
            CommonSchemas.safeString.safeParse("<script>alert(1)</script>")
                .success,
        ).toBe(false);
    });
    ...
    it("validates IP address", () => {
        expect(CommonSchemas.ipAddress.safeParse("192.168.1.1").success).toBe(
            true,
        );
    ...
    it("validates user agent", () => {
        expect(CommonSchemas.userAgent.safeParse("Mozilla/5.0").success).toBe(
            true,
        );
        expect(CommonSchemas.userAgent.safeParse("sqlmap/1.0").success).toBe(
            false,
        );
    });
});
...
describe("requestSchemas", () => {
    ...
    it("validates updateProduct schema", () => {
        ...
        expect(RequestSchemas.updateProduct.safeParse(valid).success).toBe(
            true,
        );
        expect(
            RequestSchemas.updateProduct.safeParse({ id: "not-a-uuid" })
                .success,
        ).toBe(false);
    });
    ...
    it("validates searchQuery schema", () => {
        expect(
            RequestSchemas.searchQuery.safeParse({ q: "apple" }).success,
        ).toBe(true);
...
describe("sanitization", () => {
    it("sanitizes string", () => {
        ...
        expect(sanitizeString("javascript:alert(1)")).not.toContain(
            "javascript:",
        );
        expect(sanitizeString("onclick=doSomething()")).not.toContain(
            "onclick=",
        );
    });
    it("sanitizes email", () => {
