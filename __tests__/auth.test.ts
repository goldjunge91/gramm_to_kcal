...
describe("Better Auth Configuration", () => {
    it("should have IP address fallback configured for rate limiting", () => {
        ...
        expect(config.advanced?.ipAddress?.ipAddressHeaders).toContain("x-forwarded-for");
        expect(config.advanced?.ipAddress?.ipAddressHeaders).toContain("x-real-ip");
        expect(config.advanced?.ipAddress?.ipAddressHeaders).toContain("cf-connecting-ip");
    });
