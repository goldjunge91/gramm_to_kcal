...
class MobileOfflineStorage {
  ...
  async getProducts(userId: string, limit = 50): Promise<Product[]> {
    ...
    return products
      .filter((p: Product & { lastUsed: number }) => p.userId === userId && !p.isDeleted)
  ...
  private async cleanupIfNeeded(): Promise<void> {
    ...
    if (currentUsage > state.maxStorage * 0.8) { // 80% threshold
  ...
  private async performCleanup(): Promise<void> {
    ...
    const sortedProducts = products.sort((a: { lastUsed: number }, b: { lastUsed: number }) => b.lastUsed - a.lastUsed)
  ...
  private async calculateStorageUsage(): Promise<number> {
    ...
    for (const storeName of stores) {
      ...
      totalSize += items.reduce((sum: number, item: any) => sum + JSON.stringify(item).length, 0)
    }
    
    return totalSize
  }

  async addToSyncQueue(operation: any, priority: 'high' | 'normal' | 'low' = 'normal'): Promise<void> {
    if (!this.db) await this.init()
