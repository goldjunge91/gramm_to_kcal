...
export class MobileSyncManager {
    ...
    private handleOffline(): void {
        this.isOnline = false
        this.pauseSync()
        toast.info('Working offline - changes will sync when online', { duration: 3000 })
    }
    ...
    private async processMobileSyncQueue(): Promise<void> {
        ...
        const highPriorityItems = queue.filter(item => item.priority === 'high').slice(0, 10)
        ...
        if (this.networkType === 'wifi' || this.networkType === '4g') {
            const normalItems = queue.filter(item => item.priority === 'normal').slice(0, 5)
    ...
    private async syncIngredient(supabase: any, item: SyncQueueItem): Promise<void> {
        switch (item.operation) {
            case 'create':
                await supabase.from('ingredients').insert(item.data)
                break
            case 'update':
                await supabase.from('ingredients').update(item.data).eq('id', item.data.id)
                break
            case 'delete':
